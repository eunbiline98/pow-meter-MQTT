...
#define PhiloinDMD_NUM_COLUMNS 32
#define PhiloinDMD_NUM_ROWS 16

#define PhiloinDMD_REFRESH_US 100

PhiloinDMD::PhiloinDMD(int widthPanels, int heightPanels)
    : Bitmap(widthPanels * PhiloinDMD_NUM_COLUMNS, heightPanels * PhiloinDMD_NUM_ROWS), _doubleBuffer(false), phase(0), fb0(0), fb1(0), displayfb(0), lastRefresh(millis())
{
    fb0 = displayfb = fb;
...
void PhiloinDMD::setDoubleBuffer(bool doubleBuffer)
{
...
void PhiloinDMD::swapBuffersAndCopy()
{
    swapBuffers();
    if (_doubleBuffer)
...
void dispinit(void)
{
    ...
    os_timer_arm_us(&dispTimer, PhiloinDMD_REFRESH_US, true);
}

void PhiloinDMD::loop()
{
    if (tickOccured == true)
    {
        tickOccured = false;
...
void PhiloinDMD::begin()
{
    ...
    while (jsh--)
    {
        if (jsh == DMD_PIN_PHASE_LSB || jsh == DMD_PIN_OUTPUT_ENABLE || jsh == DMD_PIN_PHASE_MSB || jsh == DMD_PIN_LATCH)
        {
            digitalWrite(jsh, 0);
            pinMode(jsh, OUTPUT);
        }
...
void PhiloinDMD::setBrightness(uint16_t b)
{
...
PhiloinDMD::Color PhiloinDMD::fromRGB(uint8_t r, uint8_t g, uint8_t b)
{
    if (r || g || b)
        return White;
    else
